...
public final class VectorInputFactory {
    ...
    public static VectorInput vectorInput(float...values) {
        return VectorInput.newBuilder().setDense(DenseVector.newBuilder().addAllData(Floats.asList(values)))
            .build();
    }
    /**
    ...
    /**
     * Creates a {@link VectorInput} from a nested list of floats representing a multi
     * vector
     *
    ...
    public static VectorInput multiVectorInput(List < List < Float >> vectors) {
        List < DenseVector > denseVectors = vectors.stream()
            .map(v -> DenseVector.newBuilder().addAllData(v).build())
            .collect(Collectors.toList());
        return VectorInput.newBuilder()
            .setMultiDense(MultiDenseVector.newBuilder()
                .addAllVectors(denseVectors)
                .build())
            .build();
    }
    /**
     * Creates a {@link VectorInput} from a nested array of floats representing a multi
     * vector
     *
